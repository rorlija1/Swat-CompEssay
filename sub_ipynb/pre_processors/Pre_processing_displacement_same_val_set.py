...
def calculate_dx(t_f_dump, t_0_dump):
...
def process_lammps_dump(lammps_dump_file, wall_file, t_f_dump, t_0_dump, cutoff_distance, train_fraction=0.9, val_fraction=0.1):
    ...
    x_displacements = calculate_dx(t_f_dump, t_0_dump)
    ...
    n_val = int(val_fraction * n_atoms)
    """
    NOTE: IF YOU WOULD NOT LIKE THE SAME
    VALIDATION ATOMS ACROSS TRIALS,
    CHANGE THE SEED BELOW TO 'None'
    ALSO, SEE THE RNG INSTANCE FOR TRAIN
    AND VAL TO PRODUCE CONSISTENT SNAPSHOTS.
    """
    rng = np.random.default_rng(seed=222)
    # always use the same validation set with random seed
    val_indices = rng.choice(n_atoms, n_val, replace=False)
    remaining_indices = np.setdiff1d(np.arange(n_atoms), val_indices)
    # randomly choose training set from available atoms
    rng = np.random.default_rng(seed=222)
    training_indices = rng.choice(remaining_indices, n_training, replace=False)
    
    # safeguard against overlap between train and validation sets
    assert len(np.intersect1d(training_indices, val_indices)) == 0, "Oops! There exist common training and validation indices!"
...
# Example usage:
process_lammps_dump("MD_Data/confdumpallelasticMD0.data", "MD_Data/wallpartidtype", "MD_Data/confdumpallelasticMD10000.data", "MD_Data/confdumpallelasticMD0.data", cutoff_distance=5.0, train_fraction=0.8, val_fraction=0.1)
