# This is non-interactive and takes its input from text file called gnn_input.txt
# modified from Amy's July4 modifications of gnn_elegant.py from Rowan
# Imports
import torch
...
with open("gnn_input.txt", "r") as f:
    lines = [line.strip() for line in f.readlines()]

print("Reading gnn_input file...")
task_input = lines[3]
snapshot_input = lines[6]
subfolder_name = lines[9]
resume_input = lines[12]
restart_model_input = lines[14]
num_epochs = int(lines[16])
savedmodel_input = True if lines[18].lower() == "true" else False
...
MP_layers = 12 # num. of message-passing layers
...
def restart_prompt(response, model_choice):
...
def write_parameter_summary(output_dir, task, snapshot_path, resume_choice, model_choice, num_epochs, savedmodel_save,
                            MP_layers, MLP_dim, p, dge_threshold, learning_rate, milestones, gamma_multistep, gamma_expo, plateau_factor,
    ...
    with open(summary_file, "w") as f:
        ...
        f.write(f"TASK: {task}\n")
        f.write(f"GNN version: gnn_noninteractive_refined.py\n")
...
edge_threshold = 3 # two nodes form an edge if separation shorter than threshold
...
resume, resume_path = restart_prompt(resume_input, restart_model_input)
